...
const createLocalSchema = z.object({
  ...
  direccion: z.string().max(255).nonempty({ message: "La dirección es requerida" }),
  categoria_culinaria_id: z.string().nonempty({ message: "Seleccione una categoría" }),
  encargado_username: z.string().min(5, { message: "Mínimo 5 caracteres" }).max(50, { message: "Máximo 50 caracteres" }).nonempty({ message: "El nombre de usuario es requerido" }),
  encargado_email: z.string().email({ message: "Email inválido" }).nonempty({ message: "El correo electrónico es requerido" }),
  encargado_tel: z.string().regex(/^\d{1,4}-\d{7,10}$/, { message: 'Teléfono inválido' }).nonempty({ message: "El teléfono es requerido" }),
});

export default function CrearLocalForm({ categorias, empresa, fetchLocales }: CreateLocalFormProps) {
    ...
    try {
      const encargadoResponse = await axios.post(
        'http://127.0.0.1:8000/api/register',
        {
          ...
          password: empresa.nombre + data.encargado_username + new Date().getFullYear(), // Set a default password or prompt user to set one later
          rol: 3,
        }
      );
    ...
    <Card>
      ...
      <CardContent>
        <Form {...createLocalForm}>
          <form onSubmit={createLocalForm.handleSubmit(handleCreateLocal)} className='flex flex-col gap-4'>
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormMessage>{createLocalForm.formState.errors.nombre?.message}</FormMessage>
                </FormItem>
              )}
            />
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormMessage>{createLocalForm.formState.errors.direccion?.message}</FormMessage>
                </FormItem>
              )}
            />
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormMessage>{createLocalForm.formState.errors.categoria_culinaria_id?.message}</FormMessage>
                </FormItem>
              )}
            />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormMessage>{createLocalForm.formState.errors.encargado_username?.message}</FormMessage>
                </FormItem>
              )}
            />
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormMessage>{createLocalForm.formState.errors.encargado_email?.message}</FormMessage>
                </FormItem>
              )}
            />
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormMessage>{createLocalForm.formState.errors.encargado_tel?.message}</FormMessage>
                </FormItem>
              )}
            />
