...
import { Locales, TramoHorario, Comentario, FotoLocal, Producto, User } from '@/lib/interfaces';
...
const EsquemaComentario = z.object({
    comentario: z
        ...
        .max(160, { message: "El comentario no debe tener más de 160 caracteres." }),
...
const DetallesLocal: React.FC = () => {
    ...
    function obtenerUsuario() {
        if (isLoggedIn) {
            axios.get<User>('http://localhost:8000/api/mi_usuario', {
                headers: {
    ...
    function onSubmit(data: z.infer<typeof EsquemaComentario>) {
        ...
        axios.post(`http://127.0.0.1:8000/api/crear_comentarios/local/${local?.id}`, datosFormulario, {
    ...
    const productosPorCategoria = local.productos.reduce((acc: { [key: string]: Producto[] }, producto) => {
    ...
    const renderizarComentarios = (comentarios: Comentario[], nivel = 0) => {
        ...
        return comentarios.map((comentario: Comentario) => (
            <div key={comentario.id} className={`flex flex-col items-start justify-center mt-4 box-border bg-gray-100 p-4 w-full mb-4 border rounded max-w-2xl`} style={{ marginRight: `${margenDerecho}px` }}>
                ...
                <p><strong>Puntuación:</strong> <Rating style={{ maxWidth: 150 }} itemStyles={estilosPersonalizados} radius='large' spaceBetween='small' spaceInside='small' value={comentario.estrellas} readOnly /></p>
                {isLoggedIn && (
                    <Button onClick={() => setComentarioAResponder(comentario)} className="bg-[#e67e22] text-white font-bold py-2 px-4 rounded mt-4">Responder</Button>
                )}
                {comentario.respuestas && renderizarComentarios(comentario.respuestas, nivel + 1)}
            </div>
        ));
    };
    ...
    return (
        <div className="flex flex-col space-y-10">
            <Link to="/" className="text-blue-500 flex"><ChevronLeft /> Volver</Link>
            <h1 className="text-2xl font-bold flex items-center"><span>{local.nombre}</span> <span className='bg-gray-200 px-2 py-1 rounded ml-2 flex items-center'><Star fill='#EAB39D' className="text-yellow-500 w-5 h-5" />{calcularMediaLikes(local.comentarios)}</span></h1>
            <div className="mt-4">
                <Carousel className='w-full max-w-2xl'>
                    <CarouselContent>
                        {local.fotos.map((foto: FotoLocal, index) => (
                            <CarouselItem key={index}>
                                <img className='max-h-30 min-h-28 w-full' src={"http://localhost:8000" + foto.imagen} alt={`Imagen ${index + 1}`} />
                            </CarouselItem>
                        ))}
                    </CarouselContent>
                    <CarouselPrevious />
            ...
            <div className="mt-4">
                ...
                <p><strong>Horario:</strong> {local.horarios.map(horario => `${horario.hora_apertura} - ${horario.hora_cierre}`).join(', ')}</p>
            ...
            <div className="mt-4">
                ...
                {local.tramos_horarios.map((tramo: TramoHorario, index) => (
                    <div key={index} className="flex flex-col items-start justify-center bg-gray-100 p-4 w-full">
                        <p><strong>{tramo.nombre}:</strong> {tramo.h_inicio} - {tramo.h_final}</p>
                        <p><strong>Máximo de Clientes:</strong> {tramo.clientes_maximos}</p>
                    </div>
                ))}
            </div>
            <div className="flex justify-between">
                <Link to={`/reservar/${local.id}`} className="bg-[#e67e22] text-white font-bold py-2 px-4 rounded mt-4">Reservar</Link>
                <Dialog>
                    <DialogTrigger><Button className="bg-[#e67e22] text-white font-bold py-2 px-4 rounded mt-4">Productos</Button></DialogTrigger>
                    <DialogContent>
                        <DialogHeader>
            ...
            {isLoggedIn && (
                <div className='p-4 bg-gray-100 border rounded'>
                    <Form {...formulario}>
                        <form onSubmit={formulario.handleSubmit(onSubmit)} className="space-y-6">
                            <FormField
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <Rating style={{ maxWidth: 250 }} itemStyles={estilosPersonalizados} radius='large' spaceBetween='small' spaceInside='large' {...field} />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}
                            />
                            <Button type="submit" className="bg-[#e67e22] text-white font-bold py-2 px-4 rounded mt-4">Comentar</Button>
                        </form>
                    </Form>
                </div>
            )}
            ...
            <Dialog open={!!comentarioAResponder} onOpenChange={() => setComentarioAResponder(null)}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Responder a Comentario</DialogTitle>
                        <DialogDescription>
                            Responder al comentario de {comentarioAResponder?.usuario.username}
                        </DialogDescription>
                    </DialogHeader>
                    <Form {...formularioRespuesta}>
                        <form onSubmit={formularioRespuesta.handleSubmit(onSubmitRespuesta)} className="space-y-6">
                            <FormField
                            ...
                            />
                            <Button type="submit" className="bg-[#e67e22] text-white font-bold py-2 px-4 rounded mt-4">Responder</Button>
                        </form>
                    </Form>
                </DialogContent>
            </Dialog>
